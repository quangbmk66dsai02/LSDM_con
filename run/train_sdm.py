...
def validate():
    ...
    with torch.no_grad():
        ...
        n_steps = 0
        #given_objs, given_cats: set of pc and cat of objs that already present in the scene
...
if __name__ == '__main__':
    ...
    if datatype == "proxd":
        ...
        train_dataset = HUMANISE(train_data_dir, max_frame=max_frame, fix_orientation=fix_ori,
                                    step_multiplier=1, jump_step=jump_step)
        train_data_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=num_workers)
        valid_dataset = HUMANISE(valid_data_dir, max_frame=max_frame, fix_orientation=fix_ori,
                                    step_multiplier=1, jump_step=jump_step)
        valid_data_loader = DataLoader(valid_dataset, batch_size=2, shuffle=True, num_workers=num_workers)
